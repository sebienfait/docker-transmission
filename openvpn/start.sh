...
openvpn_opts="--auth-user-pass ${openvpn_login_file} --script-security 2 --up /etc/openvpn/up.sh --down /etc/openvpn/down.sh ${OPENVPN_OPTS}"
...
exec openvpn --config "${openvpn_config_file}" ${openvpn_opts} || rm -f ${openvpn_login_file} && exit $?
